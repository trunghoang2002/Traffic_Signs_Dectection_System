...
class TrafficSignDetection:
    ...
    def update(
        ...
        if self.running:
            ...
            # Directories
            save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)  # increment run (exp1, exp2, ..)
            if save_img:
                (save_dir / 'labels' if save_txt else save_dir).mkdir(parents=True, exist_ok=True)  # make save dir
            ...
            label_old = None
            start_time, end_time = timeit.default_timer()-2, 0
            for path, im, im0s, vid_cap, s in dataset:
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    if len(det):
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            ...
                            if save_img or save_crop or view_img:  # Add bbox to image
                                ...
                                if label_old != label and execution_time > 2:
                                    end_time = 0
                                    print(label, execution_time)
                                    label_old = label
                    ...
                    if view_img:
                        frame = cv2.resize(im0, (int(im0.shape[1] * 1.5), int(im0.shape[0] * 1.5)))
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weights/yolov5n_12label.pt', help='model path or triton URL')
